<?php
\Contao\System::loadLanguageFile('tl_application_form');

$table = 'tl_active_times';

$GLOBALS['TL_DCA'][$table] = [
    'config' => [
        'dataContainer' => 'Table',
        'sql' => [
            'keys' => [
                'id' => 'primary'
            ]
        ]
    ],
    'fields' => [
        'id' => [
            'sql' => ['type' => 'integer', 'length' => 10, 'unsigned' => true, 'notnull' => true, 'autoincrement' => true],
        ],
        'username' => [
            'label' => &$GLOBALS['TL_LANG']['application_form']['username'],
            'exclude' => true,
            'inputType' => 'text',
            'eval' => ['tl_class' => 'w50'],
            'sql' => ['type' => 'string', 'length' => 255, 'notnull' => true, 'default' => ''],
        ],
        'length' => [
            'label' => &$GLOBALS['TL_LANG']['application_form']['length'],
            'exclude' => true,
            'inputType' => 'text',
            'eval' => ['tl_class' => 'w50'],
            'sql' => ['type' => 'integer', 'notnull' => true, 'unsigned' => true],
        ],
        'month' => [
            'label' => &$GLOBALS['TL_LANG']['application_form']['month'],
            'exclude' => true,
            'inputType' => 'text',
            'eval' => ['tl_class' => 'w50'],
            'sql' => ['type' => 'integer', 'notnull' => true, 'unsigned' => true],
        ],
        'year' => [
            'label' => &$GLOBALS['TL_LANG']['application_form']['year'],
            'exclude' => true,
            'inputType' => 'text',
            'eval' => ['tl_class' => 'w50'],
            'sql' => ['type' => 'integer', 'notnull' => true, 'unsigned' => true],
        ],
    ]
];
